...
#Epoch 40: train==[0.04009=0.03774 + 0.00235 + 0.00000], recall=[0.13596], precision=[0.13467], ndcg=[0.18572], auc=[0.98767]
...
for line in sys.stdin:
    line = line.strip()
    epoch_id = int(line.split(' ')[4][:-1])
    recall = float(line.split('[')[2].split(']')[0])
    precision = float(line.split('[')[3].split(']')[0])
    ndcg = float(line.split('[')[4].split(']')[0])
    auc = float(line.split('[')[5].split(']')[0])
